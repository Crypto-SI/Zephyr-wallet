...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://github.com/mrcarlanthony/Zephyr-wallet"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/${PRODUCT_NAME_NO_SPACE}-${PRODUCT_VERSION}-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "zephyr_code\gui\icons\zephyr.ico"
...
Section "${PRODUCT_NAME}" SectionDE
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\zephyr"
  ...
  File /r "dist\zephyr\*.*"
  File "zephyr_code\gui\icons\zephyr.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\Zephyr-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\Zephyr-${PRODUCT_VERSION}.exe" "" "$INSTDIR\Zephyr-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\Zephyr-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\Zephyr-${PRODUCT_VERSION}.exe" 0


  ;Links zephyr: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\zephyr" "" "URL:pivx Protocol"
  WriteRegStr HKCU "Software\Classes\zephyr" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\zephyr\shell\open\command" "" "$\"$INSTDIR\Zephyr-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "Zephyr Wallet"
