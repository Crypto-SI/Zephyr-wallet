...
import zephyr
from zephyr_code.util import format_satoshis
from zephyr_code.bitcoin import is_address, COIN, TYPE_ADDRESS
from zephyr_code.transaction import TxOutput
from zephyr_code.wallet import Wallet
from zephyr_code.storage import WalletStorage
from zephyr_code.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
from zephyr_code.interface import deserialize_server
from zephyr_code.logging import console_stderr_handler
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'zephyr create'")
            exit()
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy'):
                proxy = zephyr.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
