...
# try to make console log lines short... no timestamp, short levelname, no "zephyr."
...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("zephyr."):
...
# creates a logger specifically for zephyr library
electrum_logger = logging.getLogger("zephyr")
electrum_logger.setLevel(logging.DEBUG)


def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("zephyr_log_*.log")), reverse=True)
    for f in files[keep:]:
        os.remove(str(f))
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"zephyr_log_{timestamp}_{PID}.log"
...
def configure_logging(config):
    ...
    _logger.info(f"Zephyr version: {ELECTRUM_VERSION} - "
                 f"https://github.com/PIVX-Project/PIVX - "
                 f"https://github.com/mrcarlanthony/Zephyr-wallet")
