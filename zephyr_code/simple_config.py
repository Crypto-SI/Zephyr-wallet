...
class SimpleConfig(Logger):
    ...
    def electrum_path(self):
        ...
        self.logger.info(f"zephyr directory {path}")
        return path
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'pivx':8, 'mpivx':5, 'upivx':2, 'sats':0, 'sat':0}
    ...
    def get_fee_text(self, pos, dyn, mempool, fee_rate):
        ...
        fee_rate is in sats/kbyte
        """
        if fee_rate is None:
            rate_str = 'unknown'
        else:
            rate_str = format_fee_satoshis(fee_rate) + ' sats/kB'
...
def read_user_config(path):
    """Parse and store the user config settings in zephyr.conf into user_config[]."""
    if not path:
        return {}
